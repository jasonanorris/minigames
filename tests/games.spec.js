...
test("The Wheel manages prizes, locks settings while spinning, and announces a winner", async ({
  page
}) => {
  ...
  await expect(page.locator(".wheel-prize-text")).toHaveText([
    "Yes",
    "No",
    "Try Again",
    "Try Again",
...
test("Tic Tac Toe plays in small mode, scores a win, and restarts", async ({ page }) => {
  ...
  await expect(page.locator("#tic-win-modal")).toBeVisible();
